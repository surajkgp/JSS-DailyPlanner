/* JSS DAILY PLANNER — JioStyle Pro Layout
   - All texts updated as per final table
   - Headings centered
   - Improved spacing, header with PRM info
   - Uses uploaded asset at: /mnt/data/69102280-9abc-4fe9-b2d8-2a163f8dcba5.png
*/
...
function earning(points, rate) { return (points * rate) + (points >= 3 ? 100 : 0); }

/* ---------- Header component used across screens ---------- */
function renderHeader() {
  const id = localStorage.getItem('jss_id') || '';
  const name = localStorage.getItem('jss_name') || '';
  // using the uploaded file path as visual
  const visual = '/mnt/data/69102280-9abc-4fe9-b2d8-2a163f8dcba5.png';
  return `
    <div class="card overflow-hidden mb-4">
      <div class="header-visual" style="height:90px; background-image:url('${visual}');"></div>
      <div class="p-4">
        <div class="text-center">
          <div class="text-lg font-extrabold text-indigo-800">Daily Target Planner</div>
          <div class="text-xs text-gray-500 mt-1">Plan your earnings for the remaining days in this cycle.</div>
        </div>
...
function showLoginView() {
  html('#content', `
    ...
    <div class="card p-5 mb-4">
      <h2 class="text-center text-xl font-bold text-indigo-700 mb-4">JSS Daily Planner</h2>
      ...
      <p id="loginError" class="mt-3 text-sm text-red-600 hidden">Please enter a valid 8-digit ID starting with 66.</p>
    </div>
  `);
...
function showInputView() {
  ...
  html('#content', `
    ...
    <div class="card p-5">
      ...
      <form id="calcForm" class="space-y-4">
        ...
        <button class="w-full bg-indigo-600 text-white p-3 rounded-lg font-semibold">Calculate My Daily Target</button>
      </form>
    </div>
  `);
  ...
  qs('#calcForm').onsubmit = (e) => {
    ...
    showResultsPage({
      daily, dailyEarn, requiredPoints, earned, remainingPoints, finalTotal, finalRate
    });
  };
}
  ...
  html('#content', `
    ...
    <div class="card p-5">
      <div class="flex justify-between items-center mb-4">
        ...
        <div class="text-sm text-gray-500">PRM ID: <span class="font-semibold text-gray-700">${localStorage.getItem('jss_id')}</span></div>
      </div>
      ...
      <div class="bg-indigo-50 p-4 rounded-lg mb-4 border border-indigo-100">
        ...
        <div class="grid grid-cols-2 gap-2 text-sm text-gray-700">
          ...
          <div class="pt-2 border-t text-indigo-900 font-semibold">Final Total Points</div><div class="pt-2 border-t text-right font-semibold">${r.finalTotal}</div>
        </div>
      </div>
      ...
      ${r.finalTotal > 24 ? `<div class="mt-2 p-3 bg-yellow-100 rounded-lg text-yellow-900 text-center font-semibold">You unlocked the higher rate: ₹40 per point!</div>` : ''}

      <button id="download" class="mt-6 w-full bg-green-600 text-white p-3 rounded-lg font-semibold">Download Plan as Image</button>
    </div>
  `);
